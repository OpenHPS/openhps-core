...
import 'mocha';
import { ModelBuilder, ObjectMergeNode, Model, DataFrame, DataObject, RelativeDistancePosition, Absolute2DPosition, TrilaterationNode, CallbackSinkNode, SourceMergeNode, TimeUnit, WorkerNode, DataSerializer, LengthUnit } from '../../../src';
                    ...
                    callbackNode.callback = (frame: EvaluationDataFrame) => {
                        frame.getObjects().forEach(object => {
                            if (object.uid === "tracked") {
                                let calculatedLocation: Absolute2DPosition = object.getPosition() as Absolute2DPosition;
                                // Accurate control location
                                const expectedLocation: Absolute2DPosition = frame.evaluationObjects.get(object.uid).getPosition() as Absolute2DPosition;
                    ...
                    callbackNode.callback = (data: EvaluationDataFrame) => {
                        data.getObjects().forEach(object => {
                            if (object.uid === "tracked") {
                                let calculatedLocation: Absolute2DPosition = object.getPosition() as Absolute2DPosition;
                                // Accurate control location
                                const expectedLocation: Absolute2DPosition = data.evaluationObjects.get(object.uid).getPosition() as Absolute2DPosition;
