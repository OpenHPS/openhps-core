...
import { Quaternion } from "../position/Quaternion";
...
export class DataObject {
    ...
    public getPosition(referenceSpace?: Space): AbsolutePosition {
        if (referenceSpace !== undefined && this._position !== undefined) {
            ...
            const orientation = transformedPosition.orientation.toEulerVector();
            orientation.push(1);
            ...
            transformedPosition.orientation = Quaternion.fromEulerVector(math.multiply(orientation, invRotationMatrix));
    ...
    public setPosition(position: AbsolutePosition, referenceSpace?: Space) {
        if (referenceSpace !== undefined) {
            ...
            // Transform the orientation (rotation)
            transformedPosition.orientation = Quaternion.fromEulerVector(math.multiply(orientation, referenceSpace.rotationMatrix));
