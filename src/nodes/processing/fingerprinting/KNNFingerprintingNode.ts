...
export class KNNFingerprintingNode<InOut extends DataFrame> extends FingerprintingNode<InOut> {
    ...
    private _onBuild(modelBuilder: ModelBuilder<any, any>): Promise<void> {
        return new Promise<void>((resolve, reject) => {
            ...
            if (this._service.dataService.isReady()) {
                this._service
                    .onUpdate()
                    .then(() => {
                        this._updateCache();
                ...
                this._service.dataService.once('ready', () => {
                    this._service
                        .onUpdate()
                        .then(() => {
                            this._updateCache();
        ...
        return new Promise((resolve) => {
            this._cacheObjects.forEach((relativeObject) => {
                if (!dataObject.hasRelativePosition(relativeObject)) {
                    ...
                    dataObject.addRelativePosition(relativeLocation);
                }
            });
            ...
            if (this.options.naive) {
                ...
                results = results
                    ...
                    .map((v: [any, number]) => [DataSerializer.deserialize(v[0]), v[1]]);
            } else {
            ...
            const predictedPosition = results[0][0];
            predictedPosition.fromVector(point);
            dataObject.setPosition(predictedPosition);
            resolve(dataObject);
        });
    }
}
    ...
    public nearest(point: number[], count = 1, maxDistance?: number): Array<[AbsolutePosition, number]> {
        ...
        for (let i = 0; i < Math.min(count, bestNodes.content.length); i += 1) {
            if (bestNodes.content[i][0]) {
                result.push([
                    DataSerializer.deserialize(bestNodes.content[i][0].fingerprint.location),
