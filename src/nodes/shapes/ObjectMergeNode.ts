...
export class ObjectMergeNode<InOut extends DataFrame> extends FrameMergeNode<InOut> {
    ...
    constructor(filterFn: (object: DataObject, frame?: InOut) => boolean, groupFn: (frame: InOut) => any, timeout: number, timeoutUnit: TimeUnit) {
        super((frame: InOut) => frame.getObjects().filter((value: DataObject) => 
            this._filterFn(value, frame)).map(object => object.uid), 
        groupFn, 
        timeout, 
        timeoutUnit);
